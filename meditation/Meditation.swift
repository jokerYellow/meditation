...
class Meditation {
    ...
    func start() {
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: options) { (flag, error) in
            ...
            content.body = "完成一个番茄钟，休息一下吧"
            let request = UNNotificationRequest.init(identifier: id, content: content, trigger:
                UNTimeIntervalNotificationTrigger.init(timeInterval: self.config.workTime.timeinterval, repeats: false))
    ...
    func haveAbreak(){
        self.state = .isBreak(times: self.state.times,time: self.state.times > self.config.workPoint ? self.config.longBreakTime : self.config.breakTime ,startDate: Date())
    }
